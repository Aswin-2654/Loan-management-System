...
class ApplyVehicleLoan(ApplyPersonalLoan):
    vehicle_type: str = Field(
        ...,
        regex=r"(?i)^(two-wheeler|four-wheeler|commercial)$"
    )
...
class ApplyEducationLoan(BaseModel):
    bank_account_number: str = Field(..., min_length=9, max_length=18, regex=r"^\d+$")
    full_name: str = Field(..., min_length=3, max_length=100)
    pan_number: str = Field(..., regex=r"^[A-Z]{5}[0-9]{4}[A-Z]$")
    ...
    guarantor_phone: Optional[str] = Field(None, regex=r"^\+?\d{10,15}$")
    guarantor_pan: Optional[str] = Field(None, regex=r"^[A-Z]{5}[0-9]{4}[A-Z]$")
