...
class KYCSubmit(BaseModel):
    ...
    marital_status: Optional[str] = Field(
        None,
        regex=r"(?i)^(single|married|divorced|widowed)$"
    )
    ...
    employment_status: Optional[str] = Field(
        None,
        regex=r"(?i)^(employed|self-employed|unemployed|student|retired)$"
    )

    employment_type: Optional[str] = Field(
        None,
        regex=r"(?i)^(private|government|business|freelancer)$"
    )
...
class KYCOut(BaseModel):
    ...
    kyc_status: str = Field(
        ...,
        regex=r"(?i)^(pending|approved|rejected)$"
    )
