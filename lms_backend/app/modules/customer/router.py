...
async def apply_personal(
    ...
    # Server-side validation: guarantor PAN must not match applicant PAN
    if guarantor_pan and pan_number and str(guarantor_pan).strip().upper() == str(pan_number).strip().upper():
        raise HTTPException(status_code=400, detail="Guarantor PAN cannot be the same as applicant PAN.")
