...
async def submit_kyc(customer_id: str, payload: dict) -> dict:
    ...
    pan_hash = _sha256_hex(pan_number) if pan_number else None
    aadhaar_hash = _sha256_hex(aadhaar_number) if aadhaar_number else None
    ...
    if pan_number and norm(pan_number) != norm(user.get("pan_number")):
        errors["pan_number"] = "PAN number does not match registration"

    if pan_number:
        pan_exists = await db.users.find_one({"pan_number": pan_number, "customer_id": {"$ne": customer_id}})
        if pan_exists:
    ...
    safe_payload.pop("pan_number", None)
    if aadhaar_number:
        # Keep raw Aadhaar in KYC record for verification-team review flows.
        # Non-verification APIs still strip it in _sanitize_kyc_doc().
        safe_payload["aadhaar_number"] = aadhaar_number
    else:
        safe_payload.pop("aadhaar_number", None)
    if pan_number:
        safe_payload["pan_hash"] = pan_hash
        safe_payload["pan_last4"] = pan_number[-4:]
        safe_payload["pan_masked"] = _mask_pan(pan_number)
    if aadhaar_number:
        safe_payload["aadhaar_hash"] = aadhaar_hash
    ...
    if existing and not pan_number:
        for key in ("pan_hash", "pan_last4", "pan_masked"):
            if existing.get(key) is not None:
                safe_payload[key] = existing.get(key)
    if existing and not aadhaar_number:
        prev_aadhaar = _normalize_aadhaar(existing.get("aadhaar_number") or existing.get("aadhar_number"))
        if prev_aadhaar:
            safe_payload["aadhaar_number"] = prev_aadhaar
        for key in ("aadhaar_hash", "aadhaar_last4", "aadhaar_masked"):
            if existing.get(key) is not None:
                safe_payload[key] = existing.get(key)
...
async def get_kyc_by_customer(customer_id: str, *, include_sensitive: bool = False):
    ...
    if include_sensitive and out is not None:
        # PAN is stored in users collection; expose only for verification flows.
        user = await db.users.find_one({"customer_id": customer_id}, {"pan_number": 1})
        pan_number = _normalize_pan((user or {}).get("pan_number"))
        if pan_number:
            out["pan_number"] = pan_number
    return out
...
async def get_verification_dashboard(page: int = 1, limit: int = 50):
    ...
    return {
        "pending_kyc": [_sanitize_kyc_doc(k) for k in pending_kyc],
        ...
        "processed_kyc": [_sanitize_kyc_doc(k) for k in processed_kyc],
